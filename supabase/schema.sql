...
-- ─── FONCTION : create_invoice_for_user — variante SYSTÈME (service_role) ─────
-- Mêmes garanties que create_invoice mais avec user_id EXPLICITE (pas auth.uid())
-- : utilisée par les déclencheurs serveur où le signataire est anonyme (route de
-- signature). Réservée au service_role. Miroir de la migration
-- 20260605_invoice_create_for_user.sql.
